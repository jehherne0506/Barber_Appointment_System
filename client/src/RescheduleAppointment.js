...
export default function RescheduleAppointment(){
    ...
    const [modalPage, setModalPage] = useState(3);
    ...
    const [errorModalType, setErrorModalType] = useState("errro");
    ...
    useEffect(()=>{
        ...
        if(serviceIdSelected!==null && staffIdSelected!=="" && dateSelected!==null && isAuthenticated && allServices){
            setAllTimeslot(null);
            fetchStaffTimeslot();
        }
    }, [serviceIdSelected, staffIdSelected, dateSelected, isAuthenticated, allServices, navigate]);
    ...
    async function handleSubmit(e){
        ...
        const response = await fetch("http://localhost:5000/appointment/rescheduleAppointment", {
            ...
            body: JSON.stringify({id: appointmentId, date: dateSelected, timeslot: timeslotSelected})
        });

        const result = await response.json();console.log(result)
        if(result.status === "success"){ 
            navigate("/appointment", {state: {successModalOpen: true, successModalType: "rescheduleAppointment"}})
        } else if(result.status === "fail" && result.message === "auth"){
            navigate("/auth/login", {state: {errorModalOpen: true}});
                ...
                <>
                    ...
                    <div className='bg-neutral-900 px-10 py-20 sm:p-20 flex flex-col'>
                        ...
                        </div>
                        {
                            ...
                            <AppointmentStep3 modalPage={modalPage} setModalPage={setModalPage} dateSelected={dateSelected} setDateSelected={setDateSelected} allTimeslot={allTimeslot} timeslotSelected={timeslotSelected} setTimeslotSelected={setTimeslotSelected} type="reschedule" />
                        }
