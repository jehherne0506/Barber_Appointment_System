...
export default function ConfirmRemoveUnavailableSlotModal({ confirmRemoveUnavailableSlotOpen, setConfirmRemoveUnavailableSlotOpen, unavailableSlot, handleRemoveUnavailableTimeslot }){
    ...
    return (
    <>
        {isAuthenticated && unavailableSlot && (
            <Dialog open={confirmRemoveUnavailableSlotOpen} onClose={() => {}} className="relative z-50">
                <DialogBackdrop
                    transition
                ...
                <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        ...
                        >
                            <button
                                onClick={() => { setConfirmRemoveUnavailableSlotOpen(false) }}
                                className="absolute top-4 right-4 text-gray-400 hover:text-yellow-600 transition-colors"
                            >
                                <XMarkIcon className="w-6 h-6" />
                            </button>

                            <form onSubmit={(e) => { setConfirmRemoveUnavailableSlotOpen(false); handleRemoveUnavailableTimeslot(e)}}>
                                    ...
                                    <div className="sm:flex sm:items-start">
                                        ...
                                        <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                            <DialogTitle as="h3" className="text-2xl font-dela text-white leading-6">
                                                Confirm Remove Unavailable Timeslot
                                            </DialogTitle>
                                            
                                            <div className="mt-4">
                                                <div className="text-gray-300 font-robotoCondensed text-base">
                                                    <p>Please confirm to delete this unavailable timeslot</p>
                                                    <p className="text-xl text-yellow-500 font-bold my-1">Date: {unavailableSlot.date?.split("T")[0]}</p>
                                                    <p className="text-xl text-yellow-500 font-bold my-1">Time: {unavailableSlot.startedAt} - {unavailableSlot.endedAt}</p>
                                                    <p className="text-xl text-yellow-500 font-bold my-1">Reason: {unavailableSlot.reason}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
